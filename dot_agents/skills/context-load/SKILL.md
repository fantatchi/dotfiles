---
name: context-load
description: 保存済みのプロジェクトコンテキストを読み込み、前回の作業状態を復帰する。セッション開始時や「前回の状態を読み込んで」「コンテキストを復帰して」「Claude からの引き継ぎを読んで」と依頼されたとき、および Claude から Codex へ作業を引き継ぐ（またはその逆の）ときに使う。`.claude/context.md`・`.claude/progress.md`・`.claude/tasks.md`（作業キュー）・`.claude/handoff.md`（他エージェントからの引き継ぎメモ）の読み込みと git 状態比較・提示を行い、すべて project-local で完結する（読み込み専用。PR 状態の実査に gh を使う）。
---
...
- 後方互換: `.claude/progress.md` がなく、かつ `{project-root}/CLAUDE.md`（または `{project-root}/AGENTS.md`）に `## 進捗マップ` セクションがある場合は、そこから抽出する（旧形式）
...
- 「進捗マップ」は `.claude/progress.md`（優先）または CLAUDE.md / AGENTS.md の `## 進捗マップ` セクション（後方互換）がある場合のみ表示する。どちらもない場合はセクションごと省略する
