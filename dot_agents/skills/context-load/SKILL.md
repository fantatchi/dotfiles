---
name: context-load
description: 保存済みのプロジェクトコンテキストを読み込み、前回の作業状態を復帰する。セッション開始時や「前回の状態を読み込んで」「コンテキストを復帰して」と依頼されたときに使う。共有正本 `.claude/context.md`、`.claude/progress.md`、tasks.md を読取専用で扱い、git の現在状態との差分を提示する。
---
