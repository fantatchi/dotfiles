...
`/obsidian-*` 系スキルを使う場合は別途 `~/ObsidianVault` の配置が必要（[作業ログ・メモを Obsidian に記録する](#作業ログメモを-obsidian-に記録する-obsidian-) を参照）。未配置でも `chezmoi apply` は警告を出して継続する。
