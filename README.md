...
| `/sonarqube` | SonarQube 新規違反（New Code Period）対応ワークフロー。API で違反棚卸し → スコープ合意（Service 配下/非 features/非 a11y 等を都度確認）→ 指摘 1 件 1 コミット → typecheck/lint/test → テンプレ準拠 PR ドラフト。前提知識は cloud-cmp の MEMORY `reference_sonarqube_*` に外出し |
