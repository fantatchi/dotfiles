...
- git がインストールされていること
- `~/ObsidianVault` が存在すること（Obsidian 連携スキルが参照する固定パス）。WSL では Windows 側の Vault へのシンボリックリンクでも可
...
Claude Code の作業内容を Obsidian Vault に自動・手動で記録する仕組み。作業ログ・調査メモ・ブログドラフトを Vault 内に Markdown で保存し、Obsidian のタグやリンク機能で後から振り返れる。

**セットアップ:**

Vault パスは `~/ObsidianVault` 固定。セットアップ手順は[クイックスタート](#クイックスタート)を参照（`chezmoi apply` 時に Vault の存在を検証するため、先に配置しておく必要がある）。
