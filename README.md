...
| `/spec-design` | 仕様書（specification / 設計ドキュメント / requirements / architecture）を書く・レビュー・改善するロール変換型スキル。判断軸 + 具体テンプレ（README / ADR Nygard・MADR / C4 / glossary）。md メイン + HTML 補足の使い分け、HTML / PDF の視覚デザイン指針（ベースカラー Blue デフォルト + 切り替え運用・「伝わるデザイン」12 原則）も内蔵（自動発動型、dashboard-design と補完関係） |
