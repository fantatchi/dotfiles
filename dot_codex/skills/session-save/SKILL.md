...
2. `vault` が有効 → `~/.codex/skills/obsidian-log/SKILL.md` を読み、その手順を実行する（引数はユーザー指定のタグがあれば渡す）
3. `vault` が無効 → このステップを skip し、完了報告で「作業ログ: skip（Vault 未設定/未配置）」と明示する

### ステップ 2: コンテキスト保存（context-save）

`~/.codex/skills/context-save/SKILL.md` を読み、その手順を無条件で実行する。context-save 自身が `.claude/context.md` 保存で完結し、tasks.md 吸い上げ・progress.md 更新などの連携は内部で resolver を見て自己 degradation する。詳細は context-save SKILL.md 側を正本とする。
