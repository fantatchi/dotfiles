...
4. **【文字数ガード｜MUST・書き込み前に必ず実行】** 追記しようとする各 Next 行について、**タイトル本体**（`- [ ] #project/<name> ` プレフィックスを除いた残り全体）の文字数を**実際に数える**。本節冒頭の規約（60-100 文字中心・**150 文字絶対上限**）に照らし、**150 文字を 1 文字でも超える行はそのまま書き込まない**。超過時は次のいずれかで必ず短縮してから書く:
   - 詳細（手順・根拠・数値・commit ID・ファイル一覧・経緯）を `context.md` / `docs/` / `progress.md` へ逃がし、Next 行には**短い見出し + 参照ポインタ**だけ残す（例: `... (詳細 context.md 6/6)`）
   - **禁止アンチパターン**: context.md の段落・箇条書きをそのまま Next 行に貼り付けない。tasks.md は**タスク管理用**であり進捗ログ・判断メモを書く場所ではない（肥大化の主因。過去に 1,500 文字超の行が混入した実例あり）
