...
- セッション全体を振り返ってから書くこと
- **同一セッションのログが既にある場合は、新規作成せず既存ファイルへ追記する**（`/session-save` はこの判断を本スキルの責務として委ねている）。判定は「その日の `20_log/YYYYMM/` に、同じセッションで自分が書いたログがあるか」。追記するのは新しい `### #N` の対話ペアで、あわせて `## 変更ファイル一覧`・`## 残課題`・frontmatter の `files_changed`・`## 概要` を**現時点の内容に更新する**。ファイル名（タイムスタンプ）は最初の作成時のまま変えない — セッション単位で 1 ファイルに保つことで、1 日に複数回保存しても断片が散らばらない
