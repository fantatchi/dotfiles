...
**主資源と連携**: このスキルは Obsidian Vault（主資源）への作業ログ書き出し専用で、Vault があれば兄弟スキル無しで動く。Vault パスは resolver `~/.claude/skills/shared/integrations.md` の `vault`（既定 `~/ObsidianVault`）から `vault-init.md` 経由で解決する。Vault が無い環境では `vault-init.md` 1 節の案内で終了する（standalone フォールバックは無い＝Vault 連携専用スキル）。それ以外の兄弟スキル連携は持たない。
...
- セッション全体を振り返ってから書くこと
- **同一セッションのログが既にある場合は、新規作成せず既存ファイルへ追記する**（`/session-save` はこの判断を本スキルの責務として委ねている）。判定は「その日の `20_log/YYYYMM/` に、同じセッションで自分が書いたログがあるか」。**同じ日でも別セッションのログは対象外**で、見分けは frontmatter の `project` と `## 概要` を読んで判断する（1 日に複数プロジェクトを触ると同じディレクトリに並ぶ。2026-08-21 に degcloud のログを書く際、同日 10:04 の**別プロジェクトのログ**が先にあり、`project: "[[cloud-cmp]]"` で別セッションと判別して新規作成した）。迷ったら追記でなく新規作成する（別セッションの記録に混ぜる方が復元しづらい）。追記するのは新しい `### #N` の対話ペアで、あわせて `## 変更ファイル一覧`・`## 残課題`・frontmatter の `files_changed`・`## 概要` を**現時点の内容に更新する**。ファイル名（タイムスタンプ）は最初の作成時のまま変えない — セッション単位で 1 ファイルに保つことで、1 日に複数回保存しても断片が散らばらない
