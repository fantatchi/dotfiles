...
スキル別のデフォルト系統 (**spec-design HTML 補足 = Vercel link blue `#0070f3`** / **dashboard-design = Blue 900 `#0017C1`**) から別系統 (Green / Orange / Light Blue / Cyan / Red 等) に切り替える場合、または業種的に複数案があり迷った場合に書く ADR。Nygard 形式の最小骨格:
...
スキル別デフォルト (spec-design 由来なら Vercel link blue、dashboard-design 由来なら Blue 900) を採用しない理由が以下にある:
...
ベースカラーとして **{{Blue / Light Blue / Cyan / Green / Orange / その他}}** を採用する。

主要 HEX 値（`shared/base-color-mapping.md` の階調マッピング準拠）:

| 用途 | 階調 | HEX |
|---|---|---|
| リンク / ハイライト | 900 | `#XXXXXX` |
| Positive | 600 | `#XXXXXX` |
| チャート Primary | 1200 | `#XXXXXX` |
| 背景アクセント | 50 | `#XXXXXX` |

Negative の扱い: {{Red 600 固定で面塗り禁止・アイコン+ラベル併用 / 例外的に別色化 = #XXXXXX}}
...
- {{例: Cyan 600 はコントラスト比 2.83:1 で 3:1 未達 → Positive 用途は強調補強が必要}}
...
- 共通リソース: `~/.claude/skills/shared/base-color-mapping.md`
- パレット HEX 全量: `~/.claude/skills/dashboard-design/references/visual-encoding.md`
```
