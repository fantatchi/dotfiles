...
**使わない場面**: GitHub へ投稿したい（built-in `/code-review --comment` または `ultra --post`）/ 作業ツリーの diff をレビューしたい（`/code-review`）/ 軽い疑問だけ（差分 1 ファイル数十行・タイポ確認）。本スキルはローカル草稿・ペルソナ・ユーザーの MEMORY 運用ルール反映が要るときに使う。
...
2. **引数パース**: URL / 番号のみ（origin から repo 推定）/ `<owner>/<repo>#<NNN>` を受ける
3. **ブランチ切替・最新化**:
...
6. **base の移動とコンフリクトの確認**（レビュー中に base が進むことがある）:
...
7. **変更単位（Change Unit）の抽出**（この PR が「何をしたか」の分解。Phase 2-5 はすべてこの ID で会話する）:
   - PR 本文の `## 対応内容` の箇条書きを**逐語のまま** `C1..Cn` に採番する。**要約しない**（要約するとペルソナがそれを PR の主張と誤認し「本文と差分が不整合」型の偽陽性を作る。memory `[[feedback_pr-review-persona-prompt-verbatim]]`）
   - `git diff --name-only $MB..HEAD` の各ファイル・各 hunk を C1..Cn のどれかへ割り当てる。**どの C にも割り当たらない hunk が残ったら `CX: PR 本文に記載のない変更` として必ず 1 単位を立てる**。回帰はここに集中する。CX が空なら「空」と書く
...
1. 「ペルソナ自動生成の指針」に従い、3-5 体のペルソナ案を生成する。**うち 1-2 体は常設の角度ペルソナ**（「常設の角度ペルソナ」節）を体数枠の**内数**で充てる — 3 体なら角度 1 + ドメイン 2、4-5 体なら角度 2 + ドメイン残り
2. 各ペルソナについて以下を提示:
...
2. 各ペルソナの prompt を作る:
   ...
   - 外部パッケージの挙動をペルソナに dist の grep で判定させる回は、**「バンドルに無い＝使っていない」と決めないこと**を書き添える。依存パッケージを外部 import して描画している場合、grep で 0 件でも実際には使われている
3. `Agent` を N 体並列起動する（subagent_type=`general-purpose`）
4. **無応答ペルソナのフォールバック**: idle 通知だけ返して所見を送ってこない体がある（`SendMessage` で 2 回催促しても返らないことがある）。その場合は**待ち続けず、そのペルソナの観点をメイン側で直接検証して Phase 4 へ進む**。所見ファイルの「メモ」に「○○ 観点はペルソナ無応答のためメインで代替検証」と明記し、代替検証で実際に見た内容（テストの `it` 全件確認 等）も書く
...
1. **MEMORY 運用ルール明示参照**（「MEMORY 運用ルール遵守」節）
...
2.5. **データの不変条件に依存する所見は、severity 確定前にユーザーへ 1 問投げる**。「その形のデータが実在するか」はコードからは決められず、DB を引ける保証も無い（ローカル DB が無い / 共有環境への SELECT が権限で止まる、はどちらも起こる）。**コード読解を尽くす前に聞く**ほうが速い。聞き方は「〜というデータは実在しますか？」の 1 問で、回答が「入らない」なら即降格、「ある」なら再現手順にその値を書ける。所見には**前提の出所**（ユーザー確認なのか実データのクエリなのか）を必ず明記する — 後から前提が変わったときに、どの指摘が復活するかを追えるようにするため。
...
2.7. **各所見に 3 値の verdict を付ける（severity 確定の前段）**。公式 `/code-review` の verify フェーズから取り込んだ判定で、**REFUTED 側に立証責任を課す**のが目的:
   ...
   **既定は PLAUSIBLE**。「speculative だから」「ランタイム状態に依存するから」を REFUTED の理由にしてはならない。並行性レース / エラーハンドラ・コールドキャッシュ・欠落オプショナルのような稀だが到達可能なパス / falsy-zero / 境界の off-by-one / リトライ嵐・部分失敗 は**現実的な状態**として扱う。ここを緩めると、MEMORY 運用ルールと合わさって実バグまで消える。
...
3. severity 確定（4 段の定義は `~/.claude/CLAUDE.md`「レビュー時の Prefix」）:
   - **【MUST】1 つの所見に複数の症状がぶら下がるときは、「確実に起きる部分」と「条件つきの部分」を分けてから severity を付ける**。原因が 1 箇所でも、症状ごとに発生頻度と害の大きさが違う。分けずに束ねると、100% 起きる軽微な症状に引きずって過大評価するか、低頻度の重い症状を見落とすかのどちらかになる。所見には**症状 × 頻度 × 害 × 再現の層**（3 層分類は MEMORY [[feedback_pr_review_repro_tiers]]）の表を置き、そのうえで「実害ベースならこの severity」と書く。既存レビュアーが別の severity を付けているなら、揃えるか否かを理由付きで明示する。
   - **由来による下げ止まり / 隔離**（2.8 で確定した由来を severity に反映する）:
     - **②新規回帰**は、**到達経路（呼び出し元・画面）が現存するなら MIDDLE 未満に下げない**。到達経路が無い（呼び出し元 0 件）なら LOW でよいが、**「前提が変われば復活するもの」に 1 行必ず残す**。ただし**下げ止まりが効くのは機能・データ・操作可能性に影響する②だけ**で、見た目だけの変化（余白・罫線・色）は LOW でよい。**下げ止まりは下限であって上げる根拠ではない**。②であることを理由に HIGH へ上げず、HIGH かどうかは発火条件の広さと害で決める
...
4. 偽陽性・降格判断:
   ...
   - MEMORY 系の除外ルール（「MEMORY 運用ルール遵守」節）に該当する所見は撤回 or LOW。ただし**抑制ルールの適用にも 2.7 の REFUTED と同じ立証責任がかかる**
...
2. **書く前に `~/.claude/skills/shared/review-output-style.md` を Read する**（`multi-persona-review` Step 5 と共通の出力規約 SSOT）。初見のジュニアエンジニアが読み通せる密度が要件で、その 4 ルールをすべて適用する。
...
- **指摘は重要度順の 1 本のフラットな並びにする**。変更単位でグループ化しない（HIGH が散らばり、「まず何を直すか」を掴むのに全体を読むことになる）
...
体数は**独立した観点の数**で決める（1 体で足りるなら 1 体）。上限の目安は差分 300 行未満で 3 体、1000 行未満で 4 体、それ以上で 5 体。100 行未満の機械的変更は短評提案（ペルソナなしも検討）。`style(` / フォーマットのみの PR も短評でよい。アクセシビリティ（SR 系）ペルソナは省略可（「MEMORY 運用ルール遵守」節）。
...
- `~/.claude/skills/shared/review-output-style.md`: 所見の書き方の SSOT。**変更単位（`C-ID`）と `由来` は PR 固有の概念なので本 SKILL 側で定義する**（SSOT 側には「verdict 行に同居させてよい・行は増やさない」の 1 行だけ置く）
- 大型 PR（> 2000 行）では各ペルソナの「担当ファイル群」で責任範囲を分割する
