...
3. ファイルパスを会話で提示
4. **context.md / tasks.md は触らない**（ユーザーが /context-save / /gtd-* で別途管理）。ただし投稿要否判断が残る CRITICAL / HIGH 所見がある場合は、`/gtd-add` での起票を**提案**する（本スキル自身は書かない。起票文例: `PR #{NNN} (<短題>) <PREFIX><件数>の投稿要否判断 (所見 <ファイル名>)` — tasks.md Next の既存 PR タスクと同形式）
