...
2. 各ペルソナの prompt を作る:
   ...
   - **アウトプット形式**: 気づいた指摘は **重要度順に全件列挙**（「重大なものだけ」「保守的に」のような件数を絞る指示は出さない。絞りは Phase 4 の裏取り 1 パスに集約する）+ 「最も効きそうな指摘トップ 3」を明示。500-800 字の目安は各指摘の説明文に掛けるもので、指摘の件数を削る根拠にしない
   - **読取専用の指示**
...
2.5. **各指摘の見出し直下に verdict 行を 1 行置く**（既存 7 件との互換は保つ。構造は変えず行を足すだけ）:

```
### HIGH: <タイトル>
- verdict: PLAUSIBLE — <根拠 1 行。CONFIRMED なら発火条件、PLAUSIBLE ならトリガの不確実性>
```

REFUTED は「確定指摘」に載せず、撤回記録（「裏取りで撤回」）に `verdict: REFUTED` と**根拠（引用した行）**を併記する。後から前提が変わったときにどの指摘が復活するかを追えるようにするため。
`## メモ` には `Phase 4.5 掃討: 新規 N 件`（またはスキップ理由）を 1 行記録する。

3. ファイルパスを会話で提示
4. **context.md / tasks.md は触らない**（ユーザーが /context-save で別途管理）。ただし投稿要否判断が残る CRITICAL / HIGH 所見がある場合は、次セッションで拾えるよう**起票を提案**する（本スキル自身は書かない。起票先はそのリポジトリの作業キュー `.claude/tasks.md` ＝ `/context-save` の担当。文例: `PR #{NNN} (<短題>) <PREFIX><件数>の投稿要否判断 (所見 <ファイル名>)`）
...
| PR の変更内容を当方の言葉で略記して「確定事実」に書く | 確定事実（特に PR 本文の主張）は原文表現を保つ。略記すると、ペルソナがそれを PR の主張と誤認し「本文と差分が不整合」型の偽陽性を生む（PR #19 の P3 が "SCSS 削除" 略記を誤読した実例、memory `[[feedback_delegate-facts-verbatim]]`） |
...
| ドメインペルソナだけ立てて削除行・呼び出し元が無担当になる | 4 章「常設の角度ペルソナ」を体数枠の内数で必ず確保する。角度 A（差分構造）は 3 体以上なら必須 |
| 「speculative だから」で実バグを REFUTED にする | Phase 4-2.7 の既定は PLAUSIBLE。REFUTED はガード行・型・不変条件を引用できるときだけ。5 章の抑制ルールにも同じ責任がかかる |
| 偽陽性カタログを finder prompt に入れて報告量を減らす | カタログは Phase 4 限定。finder 側で絞らせない設計を崩さない |
| Phase 4.5 の掃討結果を無検証で所見に載せる | 掃討候補も Phase 4 の 2 → 2.7 → 3 を通す。掃討は finder であって verifier ではない |
