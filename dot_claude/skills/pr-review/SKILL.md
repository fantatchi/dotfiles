...
3. severity 確定:
   ...
   - **由来による下げ止まり / 隔離**（2.8 で確定した由来を severity に反映する）:
     - **②新規回帰**は、**到達経路（呼び出し元・画面）が現存するなら MIDDLE 未満に下げない**。到達経路が無い（呼び出し元 0 件）なら LOW でよいが、**「前提が変われば復活するもの」に 1 行必ず残す**。ただし**下げ止まりが効くのは機能・データ・操作可能性に影響する②だけ**で、見た目だけの変化（余白・罫線・色）は LOW でよい
