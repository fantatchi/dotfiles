...
- **差分の場所**: `dialog-ex.tsx:112-283, 341-362`
- **呼び出し元・到達する画面**: 一括編集ダイアログ 2 箇所（`deg` / `quality`）
- **人が見るならここ**: 一括編集を開く → 角をドラッグ → ブラウザズーム 175% でクランプを見る
- **指摘**: HIGH-1 / MIDDLE-1 / MIDDLE-3 / LOW-1
...
（同じ形で C1 → C2 → … → CX の順。各節の中は CRITICAL → HIGH → MIDDLE → LOW 順）
...
<この回の結論を左右した実行だけを表で。考察は書かない。網羅的な grep 件数は変更単位の「呼び出し元」行へ移す>
...
- **`## 指摘一覧` のフラット表は廃止**し、**変更単位（C）の節に指摘をぶら下げる**。読み手の主動線は「どの改修に対する指摘か」で、重要度順の動線は「直すなら効く順」（上位 3 件）と各節内の並びで担保する。**同じ指摘を 2 箇所に書かない**
- **`## 変更単位ごとの所見` の各節のメタは 4 行以内**（差分の場所 / 呼び出し元・到達する画面 / 人が見るならここ / 指摘の ID 列挙）。ここで指摘の内容を説明し始めない（同じことを 2 回書くとファイルが倍になる）
- **`対象変更` は見出し階層で表現し、`由来` は `verdict` 行に同居させる**（`CONFIRMED ／ ②新規回帰 — <根拠>`）。指摘本文で「これは本 PR が新設した回帰で」と説明し始めない（1 指摘 6 行の規約が壊れる）
- **③既存債務は「PR 外」表に 1 行だけ置き、詳細ブロックを書かない**。これが本改訂の引き算の主軸で、変更単位の節が増えるぶんを相殺する
...
| 指摘は正しいが「どの改修のせいか」が読み手に分からない | 変更単位（C）の見出しと由来ラベルという**構造**で答える。指摘本文に「これは本 PR が新設した回帰で」と説明を足して解決しようとしない（1 指摘 6 行が壊れ、しかも全件には書けないので不揃いになる） |
...
- `~/.claude/skills/shared/review-output-style.md`: **所見の書き方の SSOT**。Phase 5 の冒頭で Read する（`multi-persona-review` Step 5 と共通）。1 指摘 4 項目 / 用語の言い換え / 再現手順の層判定 / 「聞ける項目」への退避。**`対象変更`（変更単位）と `由来` は merge-base と PR 本文を要する PR 固有の概念なので本 SKILL 側で定義する**（SSOT 側には「verdict 行に同居させてよい・行は増やさない」の 1 行だけ置く）
