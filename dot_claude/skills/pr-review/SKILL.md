...
6. **投稿可否は聞かない**（過去 7 件と同じ運用、必要なら別途「投稿用整形」依頼可）
...
| 所見が長すぎて初見で読む気を失う | Phase 5 の構造（結論 3 行 → 効く順 3 件 → 指摘一覧の表 → 各 6 行 → 聞ける項目）を守る。詳細は消さず `shared/review-output-style.md` のルール 4 で見出しだけ残す |
| 機構の解説・ルール実装の引用を所見に書き込む | 結論を左右した事実だけ「検証」節に 1 行。解説は「聞けば出せる項目」へ |
| ユーザー操作で再現できる指摘に再現手順が無い | ルール 3 の層判定を**指摘ごとに**行う。①画面のみ なら番号付き手順を必ず書く（省略不可） |
| 保守性・コメント誤りの指摘に無理やり再現手順をでっち上げる | 「画面からは再現しない（理由）」と 1 行書けばよい。空欄にはしない |
| 所見は短くしたのに会話の要約が長い | Phase 5-4 のとおり会話側も同じルールで書く |
