---
name: ks-name
description: 日本語から土木業界向けの識別子名（変数・クラス・関数・型など）を生成する。**`/ks-name` での手動呼び出し専用**（`disable-model-invocation: true` で自動起動しない）。土木プロジェクト（cloud-dsc / ks-*）で実装フェーズに入ったタイミングで利用する想定。命名規則や名前の相談、「この日本語を変数名にしたい」「クラス名どうする？」といった質問は `/ks-name` で明示的に呼び出すこと。
...
0. **`./glossary.md` 冒頭の「命名上の注記」を必ず先に読む**（多義語の使い分け: 確認 / 計測 / Standard 衝突 / enum 値 / 否定接頭辞）
1. このプロジェクト内の既存コードを検索し、同じ概念や類似の概念がどう命名されているか確認する
...
2. **推奨する名前**: 用途に応じて以下のケースで提案
   ...
   - 列挙型メンバ: PascalCase + 状態形（例: `ConfirmationStatus.Unconfirmed`、`Pass` ではなく `Passed`）。glossary 冒頭の「enum メンバ命名」注記に従う
