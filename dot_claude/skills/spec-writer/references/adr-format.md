...
デフォルト系統 (**spec-writer HTML 補足 = Vercel link blue `#0070f3`**) から別系統 (Blue 900 / Green / Orange / Light Blue / Cyan / Red 等) に切り替える場合、または業種的に複数案があり迷った場合に書く ADR。Nygard 形式の最小骨格:
...
デフォルト (Vercel link blue) を採用しない理由が以下にある:
...
主要 HEX 値（`base-color-mapping.md` の階調マッピング準拠）:
...
- ベースカラー切替ルール: `~/.claude/skills/spec-writer/references/base-color-mapping.md`
- パレット HEX 全量: `~/.claude/skills/spec-writer/references/visual-encoding.md`
```
