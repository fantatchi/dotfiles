...
`*_probe` が無いキー（`vault` など）はキー自身を probe とする。パスが設定済みでも実体が無ければ (c)（未同期・未配置）として skip する。**主資源キーの例外**: `task_store` は gtd-* の、`vault` は obsidian-* の主資源で、空・未設定なら既定値（本ファイルの値）へフォールバックし、実体が不在なら案内して終了する（連携として参照する gtd-list の転記・session-save は通常どおり skip）。
