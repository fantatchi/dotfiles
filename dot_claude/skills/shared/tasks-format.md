...
tasks.md は **Claude・Codex・Obsidian モバイルが共有する唯一のタスクストア**である。Codex 等の外部エージェントも本フォーマットと後述「書き込みプロトコル（複数 writer・MUST）」に従う writer として扱い、writer ごとのコピーは作らない。
...
tasks.md は Claude（`gtd-add` / `gtd-done` / `context-save`）・Codex・Obsidian モバイル（Sync 経由）が同時に更新し得る。共通原則（保持・最小変更・再読込・競合停止・検証・不明内容は削除しない）は `~/.claude/skills/shared/multi-writer.md` を SSOT とし、tasks.md 固有の適用を以下に定める。
...
1. **書き込み直前の再読込**: 最初の読取りから書き込みまでの間に他 writer が更新し得るため、Edit 適用の直前に tasks.md を再 Read する。内容が最初の読取りと食い違う場合、**古いスナップショットを根拠にファイル全体を上書きしない**。差分が自分の編集対象セクションと重ならなければ最新内容を基準に編集を組み直して続行し、重なる・判断できない場合は自動編集を停止してユーザーに競合を報告する
2. **重複セクションの検出**: 同名のセクション見出し（`## Inbox` / `## Next` / `## Waiting` / `## Someday` / `## Done`）が **2 回以上出現** する場合、Obsidian Sync の競合等でファイルが二重化している疑いがある。この状態でセクションを anchor に編集すると、意図しない側のコピーをサイレントに書き換える事故が起きる。全 writer スキルは編集前に見出しの出現回数をチェックし、重複を検出したら **自動編集せずユーザーに警告** し、競合解消（1 組へのマージ）を促す。やむを得ず編集する場合は「どちらのセクションに書くか」を明示してから行う
3. **重複タスクの追加禁止**: 追加しようとするタスクと同内容のタスクが既に `## Inbox` / `## Next` / `## Waiting` にある場合（部分一致で判定）、重複追加しない。既存行の存在を報告してスキップする

### 書き込み後検証（MUST）

書き戻した直後に tasks.md を再 Read し、以下を検証する（検証なしで完了としない）:

1. 5 セクション見出し（Inbox / Next / Waiting / Someday / Done）が各 1 回のままであること
2. 追記・移動したタスク行が意図した位置に存在すること
3. 追記した行のタイトル本体が文字数規則（150 文字絶対上限）を満たしていること（超過が残っていればその場で短縮して再 Edit）
