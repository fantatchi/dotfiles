...
- **重複セクションの検出（MUST）**: 同名のセクション見出し（`## Next` / `## Done` 等）が **2 回以上出現** する場合、Obsidian Sync の競合等でファイルが二重化している疑いがある。この状態でセクションを anchor に編集すると、意図しない側のコピーをサイレントに書き換える事故が起きる。スキル（`gtd-add` / `gtd-done` / `context-save`）は編集前に見出しの出現回数をチェックし、重複を検出したら **自動編集せずユーザーに警告** し、競合解消（1 組へのマージ）を促す。やむを得ず編集する場合は「どちらのセクションに書くか」を明示してから行う
