...
表示処理の前に、`## Done` セクションから**1 週間以上前のエントリを削除**する。捕捉箱は直近の Done だけを保持する運用（古い完了タスクは明示的にアーカイブせず捨てる）。月次/年次振り返り耐性が欲しい環境では、削除前に Daily Note へ転記する（連携1、`vault` があるときのみ）。

> しきい値を 2 週間から 1 週間へ縮めた（2026-07-27）。完了サマリは 1 件 1,000 字を超えることがあり、Done が溜まると `gtd-list` が毎回読むコストが跳ね上がるため（実測: 53 件で 27k トークン）。転記先の Daily Note が正本になるので、縮めても情報は失われない。**Daily Note 転記が無効な環境（`vault` 未設定）では削除が不可逆になる**点に注意。

#### 手順

1. しきい値日付を取得: `date -d "1 week ago" +%Y-%m-%d`（Linux / WSL）または `date -v-1w +%Y-%m-%d`（macOS / BSD）。どちらも失敗した場合は剪定をスキップして通常の表示処理に進む
