#!/usr/bin/env bash
# 通知 hook (Notification): 各プラットフォームで通知を表示する。
# WSL:   notification-hook.ps1 を -EncodedCommand で呼ぶ（日本語の文字化け回避）
# macOS: osascript
# Linux: notify-send
...
if command -v powershell.exe &>/dev/null; then
    ...
    PS1_FILE="$HOME/.claude/scripts/notification-hook.ps1"
    [ -f "$PS1_FILE" ] || exit 0
...
if [ "$(uname)" = "Darwin" ]; then
    osascript -e 'display notification "確認が必要です" with title "ClaudeCode"' 2>/dev/null
    exit 0
fi

# --- Linux (notify-send) ---
if command -v notify-send &>/dev/null; then
    notify-send "ClaudeCode" "確認が必要です"
fi
